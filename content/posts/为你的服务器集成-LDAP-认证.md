...
如果我们输入的用户名或者密码不正确会怎么样呢？浏览器将会坚持不懈地提示你输入用户名和密码。如果我们点击取消会怎么样呢？此时，如下图所示，它将会返回 `401` 状态码，表示认证失败：

![Nginx 集成 LDAP 效果演示-2](/posts/为你的服务器集成-LDAP-认证/集成-LDAP-认证-02.png)
...
可以注意到，两者的配置是非常相似的，自然，两者实现的效果是完全相同的，这里就不再详细展示啦！
