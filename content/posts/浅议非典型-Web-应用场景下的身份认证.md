...
description: 不过，请放心，这一切不会特别难，因为 这个类我们已经在前面用过一次： 不过，我个人感觉，这样会增加消息消费方的工作，更好的做法是采用   或者   来做统一的处理，这样，消息订阅方只需要关心消息怎么处理即可，考虑到
  Kafka 属于 Pull 模式的消息队列，这种思路不见得在性能上有多少提升，更多的是一种简化啦，算是少写一点重复代码： 其中， 内部利用 在不同线程间共享用户信息，这可以让我们在任意位置访问用户信息，因为该扩展方法中会调用一次
  方法，所以，只要在 Kafka 的消息头上维护了 字段，就可以从中解析出常用的用户信息，譬如用户名、用户角色等等
slug: 2478147871
tags:
- gRPC
- SignalR
...
![分布式事务 SAGA 模式示意图](/posts/浅议非典型-Web-应用场景下的身份认证/Kafka-Distributed-Transaction.drawio.png)
